  ...
  return (
    <>
      <div style={{ background:`linear-gradient(135deg,${NAVY},${TEAL})`, borderRadius:14, padding:"20px 24px", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
        <div>
          <div style={{ color:GOLD, fontWeight:800, fontSize:16, letterSpacing:.5 }}>QLUS — Quran Learning & Understanding Studio</div>
          <div style={{ color:"rgba(255,255,255,.6)", fontSize:12, marginTop:3 }}>Focus Islamic &amp; Western School · Integrated Quran Education Module</div>
        </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
        <Card style={{ padding:14 }}>
          ...
          {(!stats?.topMemorizers || stats.topMemorizers.length === 0) ? (
            <div style={{ color:"#94a3b8", fontSize:12, textAlign:"center", padding:20 }}>No Tahfeez data yet.<br/>Log Tahfeez sessions to see rankings.</div>
          ) : (
          ...
          {QLUS_SUBJECTS.map(sub => {
            ...
            return (
              <div key={sub} style={{ padding:"10px 12px", background:"#f8fafc", borderRadius:10, border:"1px solid #e2e8f0" }}>
                ...
                <div style={{ fontSize:10, color:TEAL, fontWeight:700 }}>{data ? `${data.completed}/${data.total} units` : "No data yet"}</div>
              </div>
            );
          })}
        </div>
